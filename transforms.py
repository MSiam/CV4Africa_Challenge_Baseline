...
def crop(img, target, region):
    ...
    o_w, o_h = img.size
    # the size dimensions of PIL image is wxh and mask,soenet are hxw
...
def resize(image, target, size, max_size=None):
    ...
    h, w = size
    if target.shape[0] > 0:
        target = F2.interpolate(
                target[:, None].float(), size, mode="nearest")[:, 0]
    else:
        target = torch.zeros((target.shape[0], h, w))
...
class InverseNormalizeTransforms:
    def __init__(self):
        self.inverse_normalize = Compose([
            Normalize([0, 0, 0], [1.0 / 0.229, 1.0 / 0.224, 1.0 / 0.225]),
            Normalize([-0.485, -0.456, -0.406], [1, 1, 1])
        ])

    def __call__(self, data):
        # import ipdb; ipdb.set_trace()
        tr = self.inverse_normalize(data, None)
        tr = torch.stack(tr[0], dim=0)
        return tr
